...
#[component]
pub fn Header(stats: RwSignal<RemoteData<Stats>>, ws_state: RwSignal<WsState>) -> impl IntoView {
    view! {
        <header class="header">
...
fn ConnectionIndicator(ws_state: RwSignal<WsState>) -> impl IntoView {
    let dot_class = move || format!("connection-dot {}", ws_state.get().css_class());
